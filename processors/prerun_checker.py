...
class PreRunChecker:
    ...
    def check_progress_report(self, file_path: Path) -> List[CheckResult]:
        ...
        results.append(CheckResult("info", f"Progress report loaded: {len(df):,} rows"))

        # At-risk count
        at_risk_col = col["at_risk"]
        at_risk_vals = {"true", "yes", "y", "1"}
        at_risk_count = (
            df[at_risk_col].astype(str).str.strip().str.lower().isin(at_risk_vals).sum()
        )
        pct = at_risk_count / len(df) * 100 if len(df) else 0
        results.append(
            CheckResult(
                "info", f"At-risk rows: {at_risk_count:,} ({pct:.1f}% of total)"
            )
        )
    ...
    def check_midterm_file(self, file_path: Path) -> List[CheckResult]:
        ...
        if missing:
            results.append(CheckResult("error", f"Missing required columns: {missing}"))
            return results
        ...
        at_risk_count = (
            ...
            .str.strip()
            .str.lower()
            .isin(MIDTERM_AT_RISK_GRADES)
            .sum()
        )
        pct = at_risk_count / len(df) * 100 if len(df) else 0
        results.append(
            CheckResult(
                "info", f"At-risk grades (C- or below): {at_risk_count:,} ({pct:.1f}%)"
            )
        )
    ...
    def check_contact_report(
        self, file_path: Path, progress_ids: Optional[set] = None
    ) -> List[CheckResult]:
        results = []
        ...
        contact_ids = set(
            normalize_student_id_series(df[id_col]).replace("", pd.NA).dropna()
        )
        results.append(
            CheckResult(
                "info",
        ...
        if progress_ids:
            ...
            if missing:
                ...
                results.append(
                    CheckResult("info", "All at-risk students have a contact record. ✓")
                )
    ...
    def check_semester_groups(
        ...
        for g in groups:
            ...
            try:
                ...
                results.append(
                    CheckResult(
                        "warning", f"Group '{name}': could not read file: {exc}"
                    )
                )

        return results

    def check_group_files(
        self, control_path: Path, group_dir: Path, progress_ids: Optional[set] = None
    ) -> List[CheckResult]:
        results = []
        ...
        results.append(
            CheckResult("info", f"Control file: {len(lines)} groups defined")
        )
        ...
        for line in lines:
            ...
            if len(parts) != 2:
                results.append(
                    CheckResult("warning", f"Malformed control file line: '{line}'")
                )
                continue
            ...
            try:
                ...
                if not ids:
                    ...
                    if progress_ids:
                        ...
                        results.append(
                            CheckResult(
                                "info", f"Group '{tab_name}': {len(ids):,} IDs loaded"
                            )
                        )
            except Exception as exc:
                results.append(
    ...
    def _load_file(self, path: Path) -> pd.DataFrame:
        ...
        if suffix == ".csv":
            for enc in ["utf-8-sig", "utf-8", "latin-1", "cp1252"]:
                try:
                    return pd.read_csv(
                        path, dtype=str, keep_default_na=False, encoding=enc
                    )
